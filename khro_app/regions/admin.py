...
from khro_app.common_info.admin import (OverideExport,OverideImportExport,
    OverideImport)
...
class LocationAdmin(OverideExport):
    ...
    fieldsets = (
        ('Primary Details',{
                'fields': ('locationlevel','parent','name','code',
                    'description','source_system')
            }),
            ('Geo-Location Info', {
                'fields': ('zone','latitude','longitude','cordinates',
                    'public_access',),
            }),
        )
    # resource_class = LocationResourceExport #added to customize fields displayed on the import window
    list_display=['name','code','locationlevel','parent','latitude','longitude']
